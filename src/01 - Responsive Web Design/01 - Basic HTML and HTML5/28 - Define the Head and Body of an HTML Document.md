...
We can add another level of organization in the HTML document within the `html` tags with the `head` and `body` elements. Any information about your page would go into the `head` tag. Then any content of the page (what displays for a user) would go into the `body` tag.

**Note**: Metadata elements, such as `link`, `meta`, `title`, and `style`, typically go inside the head element.
